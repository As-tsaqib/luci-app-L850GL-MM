...
typedef struct {
	...
	FibocomUbus *ubus;
	FibocomModem *original;
	FibocomModem *replacement;
	GCancellable *cancellable;
	struct ubus_request_data request;
	...
	gchar *physdev;
	gchar set_command[FIBOCOM_L850_COMMAND_MAX];
	guint32 earfcn;
	guint16 pci;
	guint16 band;
	gboolean has_pci;
...
#ifdef FIBOCOM_MM_L850_EXPERT
static void
cancel_l850_scan_operations(FibocomUbus *ubus, gboolean transport_lost)
{
	GList *operations;
	GList *cursor;
...
static void
cancel_l850_status_operations(FibocomUbus *ubus, gboolean transport_lost)
{
	GList *operations;
	GList *cursor;
...
static void
cancel_l850_mutation_operations(FibocomUbus *ubus,
				gboolean transport_lost)
{
	GList *operations;
	GList *cursor;
...
static struct ubus_object_type l850_object_type =
	UBUS_OBJECT_TYPE("fibocom.mm.l850", l850_methods);
#endif
...
{
	return fibocom_ubus_blob_attr_is_valid(attribute, remaining,
		name_required);
}
...
{
	return (FibocomUbus *)((gchar *)object -
		G_STRUCT_OFFSET(FibocomUbus, l850_object));
}
#endif
...
{
	return fibocom_ubus_message_is_empty(message);
}

static gboolean
parse_modem_id(struct blob_attr *message, struct blob_attr **parsed)
{
	if (!fibocom_ubus_parse_exact(message, modem_policy, __MODEM_MAX,
...
blob_string_is_canonical(struct blob_attr *attribute, size_t remaining,
			 gboolean name_required)
{
	return fibocom_ubus_blob_string_is_canonical(attribute, remaining,
		name_required);
}
...
{
	return fibocom_ubus_parse_exact(message, policy, policy_length,
		required, parsed);
}
...
sms_dedupe_prune(FibocomModem *modem)
{
	...
	for (cursor = modem->sms_dedupe->head; cursor != NULL; cursor = next) {
		...
		next = cursor->next;
		if (fibocom_sms_dedupe_is_expired(entry->expires_at, now)) {
		 ...
		 const gchar *error_code, gboolean retryable)
{
	...
	if (entry == NULL) {
		while (fibocom_sms_dedupe_evictions_required(
	...
	entry->expires_at = fibocom_sms_dedupe_expiry(g_get_monotonic_time());
}
...
static void
add_current_bands_summary(struct blob_buf *buffer, FibocomModem *modem)
{
		    ...
		    struct blob_attr *message)
{
	...
	if (!g_str_equal(fibocom_modem_composition(modem), "mbim"))
		...
		FibocomUbus *ubus, FibocomModem *modem)
{
	...
	else if (!fibocom_l850_firmware_is_allowed(
			 mm_modem_get_revision(modem->modem)))
...
static void
add_overview_capabilities(struct blob_buf *buffer, FibocomUbus *ubus,
			  FibocomModem *modem)
{
	...
	gboolean attested = fibocom_modem_attest_mutation_target(modem);
		...
		struct blob_attr *message)
{
	...
	blobmsg_add_u32(&buffer, "dedupe_capacity", FIBOCOM_SMS_DEDUPE_MAX);
	blobmsg_add_u32(&buffer, "dedupe_window_seconds",
		FIBOCOM_SMS_DEDUPE_SECONDS);
...
#ifdef FIBOCOM_MM_L850_EXPERT
static gboolean
l850_modem_has_active_mutation(FibocomUbus *ubus, FibocomModem *modem)
{
	GHashTableIter iter;
	gpointer key;
	const gchar *physdev;
...
static FibocomModem *
l850_requested_modem(FibocomUbus *ubus, struct blob_attr **parsed,
...
static gboolean
l850_firmware_allowed(FibocomModem *modem)
{
	return fibocom_l850_firmware_is_allowed(
		mm_modem_get_revision(modem->modem));
}

static gboolean l850_band_is_supported(FibocomModem *modem, guint16 band);
static L850NormalizedError l850_normalize_error(GError *error,
						gboolean timed_out,
						gboolean transport_lost);

static gboolean
l850_supported_lte_bands_available(FibocomModem *modem)
{
	struct FibocomRadioBand band_choices[MAX_RADIO_BANDS];
...
static void
l850_add_scan_capability(struct blob_buf *buffer, FibocomUbus *ubus,
			 FibocomModem *modem)
{
...
l850_add_lock_observation(struct blob_buf *buffer,
			  const struct FibocomL850LockState *state)
{
...
l850_status_ready(GObject *source, GAsyncResult *result, gpointer user_data)
{
	...
	struct FibocomL850LockState lock_state;
	L850NormalizedError normalized;
	enum FibocomL850CellParseResult parse_result;
	struct blob_buf buffer = {};
	guint32 retry_after;
	gboolean mutable;
	...
	parse_result = fibocom_l850_nvm_parse(response, strlen(response),
		&lock_state);
	if (parse_result != FIBOCOM_L850_CELL_PARSE_OK) {
...
l850_status_operation_new(FibocomUbus *ubus, FibocomModem *modem,
			  struct ubus_context *context,
			  struct ubus_request_data *request)
{
			...
			struct blob_attr *message)
{
	...
	mm_modem_command(operation->modem->modem,
		fibocom_l850_nvm_query_command(), L850_COMMAND_TIMEOUT_SECONDS,
...
static gboolean
l850_normalize_standard_cells(FibocomModem *modem, GList *items,
			      L850StandardCell *normalized, guint *n_normalized)
{
	...
	for (cursor = items; cursor != NULL; cursor = cursor->next) {
		...
		if (mm_cell_info_get_cell_type(cell) != MM_CELL_TYPE_LTE)
			continue;
		if (!MM_IS_CELL_INFO_LTE(cell) ||
		    count >= FIBOCOM_L850_CELL_MAX_RESULTS)
			return FALSE;
		lte = MM_CELL_INFO_LTE(cell);
		earfcn = mm_cell_info_lte_get_earfcn(lte);
		if (earfcn == G_MAXUINT ||
		    !fibocom_l850_earfcn_to_band(earfcn, &band) ||
...
static gboolean
l850_normalize_vendor_cells(FibocomModem *modem,
			    const struct FibocomL850CellScan *scan,
			    L850StandardCell *normalized,
			    guint *n_normalized)
{
	size_t index;

	if (scan == NULL || scan->length > FIBOCOM_L850_CELL_MAX_RESULTS)
		return FALSE;
	for (index = 0U; index < scan->length; index++) {
		const struct FibocomL850Cell *input = &scan->cells[index];
		       ...
		       gpointer user_data)
{
	...
	g_autoptr(GError) error = NULL;
	g_autofree gchar *response = NULL;
	struct FibocomL850CellScan parsed;
	L850StandardCell normalized[FIBOCOM_L850_CELL_MAX_RESULTS];
	...
	parse_result = fibocom_l850_cell_parse(response, strlen(response),
		&parsed);
	if (parse_result != FIBOCOM_L850_CELL_PARSE_OK ||
...
l850_scan_operation_new(FibocomUbus *ubus, FibocomModem *modem,
			struct ubus_context *context,
			struct ubus_request_data *request)
{
...
static void
l850_mutation_apply_cooldown(FibocomModem *modem)
{
	gint64 deadline;
...
static void
l850_mutation_release_modem(FibocomModem *modem)
{
	if (modem == NULL)
		return;
	l850_mutation_apply_cooldown(modem);
	if (modem->mutation_kind == FIBOCOM_MUTATION_L850) {
		g_clear_object(&modem->mutation_cancellable);
		modem->mutation_busy = FALSE;
		modem->mutation_kind = FIBOCOM_MUTATION_NONE;
	}
}
...
l850_mutation_operation_free(L850MutationOperation *operation)
{
	...
	fibocom_modem_unref(operation->replacement);
	fibocom_modem_unref(operation->original);
...
{
	struct blob_buf buffer = {};
	FibocomModem *modem = l850_mutation_response_modem(operation);
	void *error;
...
l850_mutation_complete_success(L850MutationOperation *operation,
			       const struct FibocomL850Cell *serving)
{
	struct blob_buf buffer = {};
				   ...
				   GObject *source)
{
	...
	if (operation->replacement == NULL ||
	    ...
	    !fibocom_modem_attest_mutation_target(operation->replacement) ||
...
l850_mutation_start_reset(L850MutationOperation *operation)
{
	...
	if (!operation->original->live ||
	    !fibocom_modem_attest_mutation_target(operation->original) ||
	...
	mm_modem_command(operation->original->modem,
		fibocom_l850_reset_command(), L850_COMMAND_TIMEOUT_SECONDS,
			...
			gpointer user_data)
{
	...
	if (!fibocom_l850_set_response_is_success(response, strlen(response))) {
...
static gboolean
l850_modem_is_registered(FibocomModem *modem)
{
			       ...
			       gboolean *ambiguous)
{
	g_autoptr(GPtrArray) modems =
		fibocom_bridge_snapshot_modems(operation->ubus->bridge);
	FibocomModem *match = NULL;
	guint index;

	*ambiguous = FALSE;
	for (index = 0U; modems != NULL && index < modems->len; index++) {
		FibocomModem *candidate = g_ptr_array_index(modems, index);
		const gchar *physdev;
		...
		if (physdev == NULL || !g_str_equal(physdev, operation->physdev) ||
		    !fibocom_modem_attest_mutation_target(candidate) ||
	...
	return match != NULL ? fibocom_modem_ref(match) : NULL;
}
	...
	if (operation->phase == L850_MUTATION_PHASE_REPROBE) {
		...
		if (replacement != NULL) {
			if (replacement->mutation_busy) {
				fibocom_modem_unref(replacement);
				operation->poll_source = 0U;
				l850_mutation_complete_failure(operation,
			...
			replacement->mutation_kind = FIBOCOM_MUTATION_L850;
			 ...
			 gpointer user_data)
{
	...
	const struct FibocomL850Cell *serving = NULL;
	enum FibocomL850CellParseResult parse_result;
	size_t index;
	...
	parse_result = fibocom_l850_cell_parse(response, strlen(response), &scan);
	if (parse_result != FIBOCOM_L850_CELL_PARSE_OK) {
		l850_mutation_complete_failure(operation,
			...
			gpointer user_data)
{
	...
	parse_result = fibocom_l850_nvm_parse(response, strlen(response),
		&lock_state);
	if (parse_result != FIBOCOM_L850_CELL_PARSE_OK ||
	    !fibocom_l850_lock_state_matches(&lock_state,
	...
	l850_mutation_replace_cancellable(operation);
	mm_modem_command(operation->replacement->modem,
		fibocom_l850_scan_command(), L850_COMMAND_TIMEOUT_SECONDS,
...
l850_mutation_operation_new(FibocomUbus *ubus, FibocomModem *modem,
			    ...
			    struct ubus_request_data *request)
{
	...
	operation->ubus = fibocom_ubus_ref_internal(ubus);
	operation->original = fibocom_modem_ref(modem);
	operation->cancellable = g_cancellable_new();
	...
	operation->timeout_source = g_timeout_add_seconds(
		...
		operation);
	modem->mutation_busy = TRUE;
	modem->mutation_kind = FIBOCOM_MUTATION_L850;
...
static int
l850_mutation_start(FibocomUbus *ubus, FibocomModem *modem,
		    ...
		    struct ubus_request_data *request)
{
	...
	command = type == L850_MUTATION_CLEAR ?
		fibocom_l850_clear_command() : operation->set_command;
		     ...
		     struct blob_attr *message)
{
	...
	guint16 band;
	gboolean has_pci;
	char command[FIBOCOM_L850_COMMAND_MAX];
	...
	if (!fibocom_l850_earfcn_to_band(earfcn, &band) ||
	...
	if (modem->mutation_busy || retry_after > 0U)
		return send_l850_error(context, request, modem, "busy", TRUE,
			retry_after);
	if (!fibocom_l850_build_set_command(earfcn, has_pci, (guint16)pci,
		       ...
		       struct blob_attr *message)
{
	...
	return l850_mutation_start(ubus, modem, L850_MUTATION_CLEAR,
		FIBOCOM_L850_CLEAR_FREQUENCY, FALSE,
		FIBOCOM_L850_PCI_WILDCARD, 0U, NULL, context, request);
}
#endif
...
connect_ubus(FibocomUbus *ubus)
{
	...
	if (!ubus->context_initialized) {
		...
		if (status != 0) {
			g_warning("cannot publish build-gated fibocom.mm.l850 object");
			ubus_shutdown(&ubus->context);
			ubus->context_initialized = FALSE;
			return FALSE;
		}
...
#ifdef FIBOCOM_MM_L850_EXPERT
	g_message("expert API published as fibocom.mm.l850 (one live-validated firmware)");
#endif
	return TRUE;
}
...
#ifdef FIBOCOM_MM_L850_EXPERT
	ubus->l850_object.name = "fibocom.mm.l850";
