...
struct CaSlot {
	bool seen;
	bool active;
	struct FibocomL850CaCarrier carrier;
};
...
validate_common_carrier(struct FibocomL850CaCarrier *carrier,
			...
			uint32_t ul_bandwidth)
{
	if (band == CA_BAND_SENTINEL || pci == CA_PCI_SENTINEL ||
		...
		return FIBOCOM_L850_CA_PARSE_SENTINEL;
	if (band == 0U || band > 85U || pci > 503U || rsrp > 97U ||
		...
		return FIBOCOM_L850_CA_PARSE_RANGE;
	...
	return FIBOCOM_L850_CA_PARSE_OK;
}

static enum FibocomL850CaParseResult
parse_primary_record(char **fields, struct FibocomL850CaCarrier *carrier)
{
	...
	for (index = 0U; index < CA_PRIMARY_FIELD_COUNT; index++) {
		...
		if (!parse_uint32_decimal(fields[index], &values[index]))
			return FIBOCOM_L850_CA_PARSE_MALFORMED;
	}
	...
	result = validate_common_carrier(carrier, values[1], values[6],
		...
		values[12], values[13]);
	if (result != FIBOCOM_L850_CA_PARSE_OK)
		return result;
	carrier->index = 1U;
		       ...
		       struct FibocomL850CaCarrier *carrier)
{
	...
	if (!parse_int32_decimal(fields[5], &sinr))
		return FIBOCOM_L850_CA_PARSE_MALFORMED;
	if (values[0] < 2U || values[0] > FIBOCOM_L850_CA_MAX_SLOTS)
		return FIBOCOM_L850_CA_PARSE_RANGE;
...
static bool
carriers_are_duplicate(const struct FibocomL850CaCarrier *left,
		       const struct FibocomL850CaCarrier *right)
{
...
fibocom_l850_ca_parse(const char *response, size_t response_length,
		      struct FibocomL850CaInfo *info)
{
	struct CaSlot slots[FIBOCOM_L850_CA_MAX_SLOTS] = {};
	char *copy;
	char *line;
	uint32_t declared_slots = 0U;
	size_t record_count = 0U;
	bool header_seen = false;
	enum FibocomL850CaParseResult result = FIBOCOM_L850_CA_PARSE_OK;

	if (info == NULL)
		return FIBOCOM_L850_CA_PARSE_MALFORMED;
	memset(info, 0, sizeof(*info));
	if (response == NULL || response_length == 0U)
		return FIBOCOM_L850_CA_PARSE_EMPTY;
	if (response_length > FIBOCOM_L850_CA_MAX_RESPONSE)
		return FIBOCOM_L850_CA_PARSE_OVERSIZED;
	...
	if (memchr(copy, '\0', response_length) != NULL) {
		result = FIBOCOM_L850_CA_PARSE_MALFORMED;
		goto out;
	}
	line = copy;
	while (line != NULL) {
		...
		if (field_count == 1U) {
			...
			if (declared_slots == 0U) {
				result = FIBOCOM_L850_CA_PARSE_RANGE;
				break;
			}
			if (declared_slots > FIBOCOM_L850_CA_MAX_SLOTS) {
				result = FIBOCOM_L850_CA_PARSE_TOO_MANY;
				break;
			}
			header_seen = true;
			line = next;
		...
		if (record_count >= declared_slots) {
			result = FIBOCOM_L850_CA_PARSE_COUNT_MISMATCH;
			break;
		}
		...
		if (slots[slot_index - 1U].seen) {
			result = FIBOCOM_L850_CA_PARSE_DUPLICATE;
			break;
		}
	...
	if (result != FIBOCOM_L850_CA_PARSE_OK)
		goto out;
	if (!header_seen)
		result = FIBOCOM_L850_CA_PARSE_EMPTY;
	else if (!slots[0].seen || !slots[0].active)
		result = FIBOCOM_L850_CA_PARSE_MISSING_PRIMARY;
...
const char *
fibocom_l850_ca_parse_result_name(enum FibocomL850CaParseResult result)
{
	switch (result) {
	case FIBOCOM_L850_CA_PARSE_OK: return "ok";
	case FIBOCOM_L850_CA_PARSE_EMPTY: return "empty";
	case FIBOCOM_L850_CA_PARSE_OVERSIZED: return "oversized";
	case FIBOCOM_L850_CA_PARSE_MALFORMED: return "malformed";
	case FIBOCOM_L850_CA_PARSE_SENTINEL: return "sentinel";
	case FIBOCOM_L850_CA_PARSE_RANGE: return "range";
	case FIBOCOM_L850_CA_PARSE_TOO_MANY: return "too_many";
	case FIBOCOM_L850_CA_PARSE_DUPLICATE: return "duplicate";
	case FIBOCOM_L850_CA_PARSE_COUNT_MISMATCH: return "count_mismatch";
	case FIBOCOM_L850_CA_PARSE_MISSING_PRIMARY: return "missing_primary";
	default: return "unknown";
	}
}

const char *
fibocom_l850_ca_query_command(void)
{
	return CA_QUERY_COMMAND;
}

uint32_t
fibocom_l850_ca_retry_after_ms(int64_t now_us,
			       int64_t last_query_completed_us)
{
	const int64_t interval =
		(int64_t)FIBOCOM_L850_CA_QUERY_COOLDOWN_SECONDS *
	...
	if (now_us < last_query_completed_us)
		return FIBOCOM_L850_CA_QUERY_COOLDOWN_SECONDS * 1000U;
