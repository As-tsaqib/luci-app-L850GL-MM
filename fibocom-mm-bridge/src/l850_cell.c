...
bool
fibocom_l850_earfcn_to_band(uint32_t earfcn, uint16_t *band)
{
	size_t index;
...
bool
fibocom_l850_band_is_supported(uint16_t band,
...
static enum FibocomL850CellParseResult
parse_cell_line(char *line, struct FibocomL850Cell *cell)
{
	...
	if (strncmp(line, "+XMCI:", 6U) != 0)
		return FIBOCOM_L850_CELL_PARSE_MALFORMED;
	...
	type = values[0];
	if (type != 4U && type != 5U)
		return FIBOCOM_L850_CELL_PARSE_UNSUPPORTED_TYPE;
	pci = values[5];
	if (pci == UINT32_MAX || values[6] == UINT32_MAX ||
	    values[9] == LTE_RSRP_SENTINEL ||
	    values[10] == LTE_RSRQ_SENTINEL)
		return FIBOCOM_L850_CELL_PARSE_SENTINEL;
	if (pci > 503U)
		return FIBOCOM_L850_CELL_PARSE_RANGE;
	if (!fibocom_l850_earfcn_to_band(values[6], &band))
		return FIBOCOM_L850_CELL_PARSE_RANGE;
	...
	return FIBOCOM_L850_CELL_PARSE_OK;
}

enum FibocomL850CellParseResult
fibocom_l850_cell_parse(const char *response, size_t response_length,
			struct FibocomL850CellScan *scan)
{
	...
	enum FibocomL850CellParseResult result = FIBOCOM_L850_CELL_PARSE_OK;

	if (response == NULL || scan == NULL || response_length == 0U)
		return FIBOCOM_L850_CELL_PARSE_EMPTY;
	if (response_length > FIBOCOM_L850_CELL_MAX_RESPONSE)
		return FIBOCOM_L850_CELL_PARSE_OVERSIZED;
	...
	while (line != NULL) {
		...
		if (strcmp(current, "OK") == 0) {
			if (ok_seen) {
				result = FIBOCOM_L850_CELL_PARSE_MALFORMED;
				break;
			}
			ok_seen = true;
			line = next;
		...
		if (ok_seen || scan->length >= FIBOCOM_L850_CELL_MAX_RESULTS) {
			result = scan->length >= FIBOCOM_L850_CELL_MAX_RESULTS ?
				FIBOCOM_L850_CELL_PARSE_TOO_MANY :
				FIBOCOM_L850_CELL_PARSE_MALFORMED;
			break;
		}
		result = parse_cell_line(current, &scan->cells[scan->length]);
		if (result != FIBOCOM_L850_CELL_PARSE_OK)
			break;
	...
	if (result == FIBOCOM_L850_CELL_PARSE_OK && scan->length == 0U)
		result = FIBOCOM_L850_CELL_PARSE_EMPTY;
	if (result != FIBOCOM_L850_CELL_PARSE_OK)
		memset(scan, 0, sizeof(*scan));
	free(copy);
	return result;
...
enum FibocomL850CellParseResult
fibocom_l850_nvm_parse(const char *response, size_t response_length,
		       struct FibocomL850LockState *state)
{
	...
	if (response == NULL || state == NULL || response_length == 0U)
		return FIBOCOM_L850_CELL_PARSE_EMPTY;
	if (response_length > FIBOCOM_L850_NVM_MAX_RESPONSE)
		return FIBOCOM_L850_CELL_PARSE_OVERSIZED;
	...
	if (parsed_count != NVM_FIELD_COUNT) {
		result = parsed_count == 0U ? FIBOCOM_L850_CELL_PARSE_EMPTY :
			FIBOCOM_L850_CELL_PARSE_MALFORMED;
		goto out;
	}
	if (values[NVM_RAT] != 3U || values[NVM_BAND_INFO] != 0U ||
	    values[NVM_PCI] > FIBOCOM_L850_PCI_WILDCARD ||
	    values[NVM_INTER_FREQUENCY] > 1U) {
		result = FIBOCOM_L850_CELL_PARSE_RANGE;
		goto out;
	}
	if (values[NVM_INTER_FREQUENCY] == 0U) {
		if (values[NVM_FREQUENCY] != FIBOCOM_L850_CLEAR_FREQUENCY ||
		    values[NVM_PCI] != FIBOCOM_L850_PCI_WILDCARD) {
			result = FIBOCOM_L850_CELL_PARSE_MALFORMED;
			goto out;
		}
		state->enabled = false;
		state->earfcn = FIBOCOM_L850_CLEAR_FREQUENCY;
		state->pci = FIBOCOM_L850_PCI_WILDCARD;
		goto out;
	}
	if (!fibocom_l850_earfcn_to_band(values[NVM_FREQUENCY], &state->band) ||
	    (values[NVM_PCI] > 503U &&
	     values[NVM_PCI] != FIBOCOM_L850_PCI_WILDCARD)) {
		result = values[NVM_FREQUENCY] == FIBOCOM_L850_CLEAR_FREQUENCY ||
	...
	state->has_pci = state->pci != FIBOCOM_L850_PCI_WILDCARD;
...
const char *
fibocom_l850_cell_parse_result_name(enum FibocomL850CellParseResult result)
{
	switch (result) {
	case FIBOCOM_L850_CELL_PARSE_OK: return "ok";
	case FIBOCOM_L850_CELL_PARSE_EMPTY: return "empty";
	case FIBOCOM_L850_CELL_PARSE_OVERSIZED: return "oversized";
	case FIBOCOM_L850_CELL_PARSE_MALFORMED: return "malformed";
	case FIBOCOM_L850_CELL_PARSE_UNSUPPORTED_TYPE: return "unsupported_type";
	case FIBOCOM_L850_CELL_PARSE_SENTINEL: return "sentinel";
	case FIBOCOM_L850_CELL_PARSE_RANGE: return "range";
	case FIBOCOM_L850_CELL_PARSE_TOO_MANY: return "too_many";
	default: return "unknown";
	}
}

const char *
fibocom_l850_state_name(enum FibocomL850State state)
{
	switch (state) {
	case FIBOCOM_L850_STATE_AVAILABLE: return "available";
	case FIBOCOM_L850_STATE_UNSUPPORTED_BUILD: return "unsupported_build";
	case FIBOCOM_L850_STATE_UNSUPPORTED_FIRMWARE: return "unsupported_firmware";
	case FIBOCOM_L850_STATE_SCAN_READY: return "scan_ready";
	case FIBOCOM_L850_STATE_LOCK_APPLIED_RESET_REQUIRED:
		return "lock_applied_reset_required";
	case FIBOCOM_L850_STATE_RESETTING: return "resetting";
	case FIBOCOM_L850_STATE_APPLIED_VERIFIED: return "applied_verified";
	case FIBOCOM_L850_STATE_CLEARED_VERIFIED: return "cleared_verified";
	case FIBOCOM_L850_STATE_REPROBE_TIMEOUT: return "reprobe_timeout";
	case FIBOCOM_L850_STATE_REGISTRATION_TIMEOUT: return "registration_timeout";
	case FIBOCOM_L850_STATE_VERIFICATION_MISMATCH:
		return "verification_mismatch";
	case FIBOCOM_L850_STATE_OUTCOME_UNKNOWN: return "outcome_unknown";
	default: return "unavailable";
	}
}

bool
fibocom_l850_state_transition_is_valid(enum FibocomL850State from,
				       enum FibocomL850State to)
{
	if (from == FIBOCOM_L850_STATE_AVAILABLE)
		return to == FIBOCOM_L850_STATE_SCAN_READY;
	if (from == FIBOCOM_L850_STATE_SCAN_READY)
		return to == FIBOCOM_L850_STATE_LOCK_APPLIED_RESET_REQUIRED ||
			to == FIBOCOM_L850_STATE_OUTCOME_UNKNOWN;
	if (from == FIBOCOM_L850_STATE_LOCK_APPLIED_RESET_REQUIRED)
		return to == FIBOCOM_L850_STATE_RESETTING ||
			to == FIBOCOM_L850_STATE_OUTCOME_UNKNOWN;
	if (from == FIBOCOM_L850_STATE_RESETTING)
		return to == FIBOCOM_L850_STATE_APPLIED_VERIFIED ||
			to == FIBOCOM_L850_STATE_CLEARED_VERIFIED ||
			to == FIBOCOM_L850_STATE_REPROBE_TIMEOUT ||
			to == FIBOCOM_L850_STATE_REGISTRATION_TIMEOUT ||
			to == FIBOCOM_L850_STATE_VERIFICATION_MISMATCH ||
			to == FIBOCOM_L850_STATE_OUTCOME_UNKNOWN;
	return false;
}

bool
fibocom_l850_firmware_is_allowed(const char *revision)
{
	return revision != NULL &&
		strcmp(revision, FIBOCOM_L850_ALLOWED_FIRMWARE) == 0;
}

bool
fibocom_l850_build_set_command(uint32_t earfcn, bool has_pci, uint16_t pci,
			       char *command, size_t command_size)
{
	...
	length = snprintf(command, command_size,
		...
		has_pci ? (unsigned int)pci :
			(unsigned int)FIBOCOM_L850_PCI_WILDCARD);
...
const char *
fibocom_l850_clear_command(void)
{
	return L850_CLEAR_COMMAND;
}

const char *
fibocom_l850_reset_command(void)
{
	return L850_RESET_COMMAND;
}

const char *
fibocom_l850_scan_command(void)
{
	return L850_SCAN_COMMAND;
}

const char *
fibocom_l850_nvm_query_command(void)
{
	return L850_NVM_QUERY_COMMAND;
}

bool
fibocom_l850_set_response_is_success(const char *response,
...
bool
fibocom_l850_lock_state_matches(const struct FibocomL850LockState *state,
				...
				uint16_t pci)
{
	...
	return state->enabled && state->earfcn == earfcn &&
		...
		state->pci == (has_pci ? pci : FIBOCOM_L850_PCI_WILDCARD);
}
