...
#define L850GL_NETWORK_DEVICE_MAX 4096U
#define L850GL_NETWORK_CONFDIR_MAX 4096U

struct BindingScan {
	unsigned int matches;
	bool unsafe_match;
	struct L850GLNetworkBinding candidate;
};

static void
binding_clear(struct L850GLNetworkBinding *binding)
{
	if (binding == NULL)
		return;
...
section_name_is_safe(const char *name, size_t *length)
{
	...
	if (!bounded_length(name, L850GL_NETWORK_SECTION_MAX, &name_length) ||
	    name_length == 0U)
		return false;
...
allowedmode_is_safe(const char *value, size_t *length)
{
	...
	if (!bounded_length(value, L850GL_NETWORK_MODE_MAX, &value_length))
		return false;
	if (value_length == 0U ||
...
bool
l850gl_network_modes_are_valid(const char *allowedmode,
				const char *preferredmode)
{
	...
	if (!bounded_length(allowedmode, L850GL_NETWORK_MODE_MAX,
			    &allowed_length) ||
	    !bounded_length(preferredmode, L850GL_NETWORK_MODE_MAX,
			    &preferred_length))
		return false;
...
static void
copy_modes(struct L850GLNetworkBinding *binding, const char *allowedmode,
	   const char *preferredmode)
{
	size_t length;
...
static enum L850GLNetworkBindingResult
scan_finish(const struct BindingScan *scan,
	    struct L850GLNetworkBinding *binding)
{
	if (scan->matches == 0U)
		return L850GL_NETWORK_BINDING_NONE;
	if (scan->matches != 1U || scan->unsafe_match)
		return L850GL_NETWORK_BINDING_AMBIGUOUS;
	*binding = scan->candidate;
	return L850GL_NETWORK_BINDING_UNIQUE;
}
			   ...
			   struct L850GLNetworkBinding *binding)
{
	...
	return bounded_length(device, L850GL_NETWORK_DEVICE_MAX,
				  ...
				  struct L850GLNetworkBinding *binding)
{
	...
	char fixture_path[L850GL_NETWORK_CONFDIR_MAX + sizeof("/network")];
	const char *package_name = "network";
	size_t confdir_length;
	enum L850GLNetworkBindingResult result = L850GL_NETWORK_BINDING_ERROR;

	if (!lookup_arguments_are_valid(device, binding))
		return L850GL_NETWORK_BINDING_ERROR;
	if (confdir != NULL) {
		if (!bounded_length(confdir, L850GL_NETWORK_CONFDIR_MAX,
				    &confdir_length) ||
...
out:
	if (result != L850GL_NETWORK_BINDING_UNIQUE)
		binding_clear(binding);
...
l850gl_network_binding_lookup(const char *device,
			       struct L850GLNetworkBinding *binding)
{
	return l850gl_network_binding_lookup_at(NULL, device, binding);
}
...
l850gl_network_modes_update_at(const char *confdir, const char *device,
				...
				struct L850GLNetworkBinding *binding)
{
	...
	enum L850GLNetworkModeUpdateResult result =
		L850GL_NETWORK_MODE_UPDATE_ERROR;

	if (!lookup_arguments_are_valid(device, binding) ||
	    !l850gl_network_modes_are_valid(allowedmode, preferredmode))
		return L850GL_NETWORK_MODE_UPDATE_INVALID;
	if (confdir != NULL) {
		if (!bounded_length(confdir, L850GL_NETWORK_CONFDIR_MAX,
				    &confdir_length) ||
		    confdir_length == 0U || confdir[0] != '/' ||
	...
	context = uci_alloc_context();
	if (context == NULL)
		return L850GL_NETWORK_MODE_UPDATE_ERROR;
	if (confdir != NULL &&
	    (uci_set_confdir(context, confdir) != UCI_OK ||
	     uci_set_conf2dir(context, NULL) != UCI_OK))
		goto out;
	...
	if (lookup_result == L850GL_NETWORK_BINDING_NONE) {
		result = L850GL_NETWORK_MODE_UPDATE_NONE;
		goto out;
	}
	if (lookup_result == L850GL_NETWORK_BINDING_AMBIGUOUS) {
		result = L850GL_NETWORK_MODE_UPDATE_AMBIGUOUS;
		goto out;
	}
	...
	if (uci_commit(context, &package, false) != UCI_OK) {
		result = L850GL_NETWORK_MODE_UPDATE_VERIFY_FAILED;
		goto out;
	}
	section = find_exact_section(context, package, device, binding->section);
	if (section == NULL ||
	    !bounded_equal(uci_lookup_option_string(context, section,
						      "allowedmode"),
			   allowedmode, L850GL_NETWORK_MODE_MAX) ||
	    !bounded_equal(uci_lookup_option_string(context, section,
						      "preferredmode"),
			   preferredmode, L850GL_NETWORK_MODE_MAX)) {
		result = L850GL_NETWORK_MODE_UPDATE_VERIFY_FAILED;
		goto out;
	}
	copy_modes(binding, allowedmode, preferredmode);
	result = L850GL_NETWORK_MODE_UPDATE_OK;

out:
	if (result != L850GL_NETWORK_MODE_UPDATE_OK)
		binding_clear(binding);
	uci_free_context(context);
	return result;
}
...
enum L850GLNetworkModeUpdateResult
l850gl_network_modes_update(const char *device, const char *allowedmode,
			     const char *preferredmode,
			     struct L850GLNetworkBinding *binding)
{
	return l850gl_network_modes_update_at(NULL, device, allowedmode,
