...
bool
l850gl_ubus_blob_attr_is_valid(const struct blob_attr *attribute,
...
bool
l850gl_ubus_blob_string_is_canonical(const struct blob_attr *attribute,
				      size_t remaining, bool name_required)
{
	const char *value;
	size_t length;

	if (!l850gl_ubus_blob_attr_is_valid(attribute, remaining,
		name_required) ||
...
bool
l850gl_ubus_rpc_session_is_valid(const struct blob_attr *attribute,
				  size_t remaining)
{
	const char *value;
	size_t index;

	if (!l850gl_ubus_blob_attr_is_valid(attribute, remaining, true) ||
	    strcmp(blobmsg_name(attribute), RPC_SESSION_NAME) != 0 ||
	    blobmsg_type(attribute) != BLOBMSG_TYPE_STRING ||
	    !l850gl_ubus_blob_string_is_canonical(attribute, remaining, true))
		return false;
	value = blobmsg_data(attribute);
...
bool
l850gl_ubus_message_is_empty(struct blob_attr *message)
{
	...
	while (remaining > 0U) {
		...
		if (!l850gl_ubus_blob_attr_is_valid(attribute, remaining, true) ||
		    session_seen ||
		    !l850gl_ubus_rpc_session_is_valid(attribute, remaining))
			return false;
		session_seen = true;
...
bool
l850gl_ubus_parse_exact(struct blob_attr *message,
			...
			struct blob_attr **parsed)
{
	...
	while (remaining > 0U) {
		...
		if (policy[index].type == BLOBMSG_TYPE_STRING &&
		    !l850gl_ubus_blob_string_is_canonical(attribute, remaining,
			true))
			return false;
