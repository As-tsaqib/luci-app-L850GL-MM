...
#define L850GL_SYSFS_ROOT "/sys"
#define L850GL_SYSFS_PREFIX "/sys/devices/"
#define L850GL_PHYSDEV_MAX 4096U
#define L850GL_COMPONENT_MAX 255U
#define L850GL_USB_ID_LEN 4U
...
physdev_length(const char *physdev, size_t *length)
{
	...
	for (i = 0; i <= L850GL_PHYSDEV_MAX; i++) {
		if (physdev[i] == '\0') {
			if (i == 0 || i > L850GL_PHYSDEV_MAX)
				return false;
			*length = i;
			return true;
		}
...
{
	if (length == 0 || length > L850GL_COMPONENT_MAX)
		return false;
...
{
	const size_t prefix_length = sizeof(L850GL_SYSFS_PREFIX) - 1U;
	char component[L850GL_COMPONENT_MAX + 1U];
	size_t path_length;
	size_t offset;
	int current_fd = -1;
	...
	if (!physdev_length(physdev, &path_length) ||
	    ...
	    memcmp(physdev, L850GL_SYSFS_PREFIX, prefix_length) != 0 ||
...
read_exact_usb_id(int device_fd, const char *attribute,
		  const char expected[L850GL_USB_ID_LEN + 1U])
{
	char value[L850GL_USB_ID_LEN + 2U];
	size_t used = 0;
	...
	if ((used == L850GL_USB_ID_LEN ||
	     (used == L850GL_USB_ID_LEN + 1U &&
	      value[L850GL_USB_ID_LEN] == '\n')) &&
	    memcmp(value, expected, L850GL_USB_ID_LEN) == 0)
		matches = true;
...
bool
l850gl_hardware_attest_l850_mbim(const char *physdev)
{
	int sysfs_root_fd;
	bool matches;

	sysfs_root_fd = open(L850GL_SYSFS_ROOT,
			     O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW);
	if (sysfs_root_fd < 0)
		return false;
...
bool
l850gl_hardware_attest_l850_mbim_at(int sysfs_root_fd, const char *physdev)
{
