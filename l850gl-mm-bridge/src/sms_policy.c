...
gboolean
l850gl_sms_recipient_is_valid(const gchar *recipient)
{
	gsize length;
	gsize index = 0U;
	gsize digits = 0U;
	...
	if (length == 0U ||
	    length > L850GL_SMS_RECIPIENT_DIGITS_MAX + 1U)
		return FALSE;
	if (recipient[0] == '+')
		index++;
	...
	return digits >= 1U && digits <= L850GL_SMS_RECIPIENT_DIGITS_MAX;
}

gboolean
l850gl_sms_outbound_text_is_valid(const gchar *value)
{
	const gchar *cursor;
	gsize bytes;
	glong characters;
	...
	if (value == NULL)
		return FALSE;
	bytes = strlen(value);
	if (bytes == 0U || bytes > L850GL_SMS_OUTBOUND_BYTES_MAX ||
	    !g_utf8_validate(value, -1, NULL))
		return FALSE;
	characters = g_utf8_strlen(value, -1);
	if (characters <= 0 ||
	    (gsize)characters > L850GL_SMS_OUTBOUND_CHARS_MAX)
		return FALSE;
	...
	guint8 digest[L850GL_SMS_SHA256_DIGEST_LEN])
{
	...
	gsize digest_length = L850GL_SMS_SHA256_DIGEST_LEN;
	...
	return digest_length == L850GL_SMS_SHA256_DIGEST_LEN;
}

gboolean
l850gl_sms_digest_equal(
	const guint8 left[L850GL_SMS_SHA256_DIGEST_LEN],
	const guint8 right[L850GL_SMS_SHA256_DIGEST_LEN])
{
	return left != NULL && right != NULL &&
		memcmp(left, right, L850GL_SMS_SHA256_DIGEST_LEN) == 0;
}
