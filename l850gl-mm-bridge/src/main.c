...
main(int argc, char **argv)
{
	...
	L850GLBridge *bridge = NULL;
	L850GLUbus *ubus = NULL;
	...
	option_context = g_option_context_new("- L850-GL ModemManager companion bridge");
	g_option_context_add_main_entries(option_context, options, NULL);
	if (!g_option_context_parse(option_context, &argc, &argv, &error)) {
		g_printerr("l850gl-mm-bridge: invalid command line\n");
		goto out;
	}
	if (argc != 1) {
		g_printerr("l850gl-mm-bridge: unexpected positional argument\n");
		goto out;
	}
	if (show_version) {
		g_print("l850gl-mm-bridge %s\n", L850GL_MM_BRIDGE_VERSION);
		exit_status = EXIT_SUCCESS;
		goto out;
	}
	if (ubus_socket != NULL && !g_path_is_absolute(ubus_socket)) {
		g_printerr("l850gl-mm-bridge: ubus socket path must be absolute\n");
		goto out;
	}
	bridge = l850gl_bridge_new();
	ubus = l850gl_ubus_new(bridge, ubus_socket);
	if (bridge == NULL || ubus == NULL) {
		g_printerr("l850gl-mm-bridge: initialization failed\n");
		goto out;
	}
	...
	l850gl_bridge_start(bridge);
	l850gl_ubus_start(ubus);
	g_message("l850gl-mm-bridge %s started%s",
		L850GL_MM_BRIDGE_VERSION, foreground ? "" :
...
out:
	...
	l850gl_ubus_free(ubus);
	l850gl_bridge_free(bridge);
