	      ...
	      struct FibocomL850CellScan *scan)
{
	...
	enum FibocomL850CellParseResult result =
		fibocom_l850_cell_parse(data, length, scan);
		  ...
		  struct FibocomL850LockState *state)
{
	...
	char *data = read_fixture(directory, name, &length);
	enum FibocomL850CellParseResult result =
		fibocom_l850_nvm_parse(data, length, state);
...
main(int argc, char **argv)
{
	...
	const struct {
		const char *name;
		enum FibocomL850CellParseResult expected;
	} invalid[] = {
		{ "invalid-type-6.txt", FIBOCOM_L850_CELL_PARSE_UNSUPPORTED_TYPE },
		{ "invalid-sentinel.txt", FIBOCOM_L850_CELL_PARSE_SENTINEL },
		{ "invalid-malformed.txt", FIBOCOM_L850_CELL_PARSE_MALFORMED },
		{ "invalid-pci-504.txt", FIBOCOM_L850_CELL_PARSE_RANGE },
		{ "invalid-earfcn.txt", FIBOCOM_L850_CELL_PARSE_RANGE },
		{ "invalid-encoding.txt", FIBOCOM_L850_CELL_PARSE_MALFORMED },
		{ "invalid-overflow.txt", FIBOCOM_L850_CELL_PARSE_MALFORMED },
		{ "invalid-required-sentinel.txt", FIBOCOM_L850_CELL_PARSE_SENTINEL },
		{ "invalid-data-after-ok.txt", FIBOCOM_L850_CELL_PARSE_MALFORMED },
	};

	assert(argc == 2);
	assert(parse_fixture(argv[1], "valid-serving-neighbor.txt", &scan) ==
		FIBOCOM_L850_CELL_PARSE_OK);
	...
	assert(fibocom_l850_cell_parse("", 0U, &scan) ==
		FIBOCOM_L850_CELL_PARSE_EMPTY);
	oversized = malloc(FIBOCOM_L850_CELL_MAX_RESPONSE + 2U);
	assert(oversized != NULL);
	memset(oversized, '0', FIBOCOM_L850_CELL_MAX_RESPONSE + 1U);
	assert(fibocom_l850_cell_parse(oversized,
		FIBOCOM_L850_CELL_MAX_RESPONSE + 1U, &scan) ==
		FIBOCOM_L850_CELL_PARSE_OVERSIZED);
	free(oversized);

	assert(fibocom_l850_earfcn_to_band(0U, &band) && band == 1U);
	assert(fibocom_l850_earfcn_to_band(1650U, &band) && band == 3U);
	assert(fibocom_l850_earfcn_to_band(66436U, &band) && band == 66U);
	assert(!fibocom_l850_earfcn_to_band(999999U, &band));
	assert(fibocom_l850_band_is_supported(3U, supported, 3U));
	assert(!fibocom_l850_band_is_supported(8U, supported, 3U));
	assert(fibocom_l850_firmware_is_allowed("18500.5001.00.05.27.30"));
	assert(!fibocom_l850_firmware_is_allowed("18500.5001.00.05.27.29"));
	assert(!fibocom_l850_firmware_is_allowed(
		"18500.5001.00.05.27.30 "));
	assert(!fibocom_l850_firmware_is_allowed(NULL));

	assert(parse_nvm_fixture(argv[1], "nvm-clear.txt", &lock_state) ==
		FIBOCOM_L850_CELL_PARSE_OK);
	assert(!lock_state.enabled);
	assert(fibocom_l850_lock_state_matches(&lock_state, true, 0U,
		false, 0U));
	assert(parse_nvm_fixture(argv[1], "nvm-locked-exact.txt",
		&lock_state) == FIBOCOM_L850_CELL_PARSE_OK);
	assert(lock_state.enabled && lock_state.has_pci &&
		lock_state.earfcn == 1325U && lock_state.pci == 381U &&
		lock_state.band == 3U);
	assert(fibocom_l850_lock_state_matches(&lock_state, false, 1325U,
		true, 381U));
	assert(!fibocom_l850_lock_state_matches(&lock_state, false, 1325U,
		true, 0U));
	...
	assert(parse_nvm_fixture(argv[1], "nvm-invalid-extra-field.txt",
		&lock_state) == FIBOCOM_L850_CELL_PARSE_MALFORMED);
	assert(parse_nvm_fixture(argv[1], "nvm-invalid-inconsistent-clear.txt",
		&lock_state) == FIBOCOM_L850_CELL_PARSE_MALFORMED);
	assert(fibocom_l850_nvm_parse("", 0U, &lock_state) ==
		FIBOCOM_L850_CELL_PARSE_EMPTY);

	assert(fibocom_l850_build_set_command(1325U, true, 381U, command,
		sizeof(command)));
	assert(strcmp(command, "AT@SIC:FREQ_LOCK(0,3,3,1,1325,381)") == 0);
	assert(fibocom_l850_build_set_command(325U, false, 0U, command,
		sizeof(command)));
	assert(strcmp(command, "AT@SIC:FREQ_LOCK(0,3,1,1,325,65535)") == 0);
	assert(!fibocom_l850_build_set_command(999999U, false, 0U, command,
		sizeof(command)));
	assert(!fibocom_l850_build_set_command(325U, true, 504U, command,
		sizeof(command)));
	assert(!fibocom_l850_build_set_command(325U, true, 0U, short_command,
		sizeof(short_command)));
	assert(strcmp(fibocom_l850_clear_command(),
		"AT@SIC:FREQ_LOCK(0,3,255,0,65535,65535)") == 0);
	assert(strcmp(fibocom_l850_reset_command(), "AT+CFUN=15") == 0);
	assert(strcmp(fibocom_l850_scan_command(), "AT+XMCI=1") == 0);
	assert(strcmp(fibocom_l850_nvm_query_command(),
		"AT@NVM:DYN_CPS.NAS_ASM.FREQ_LOCK_PARAMS.*??") == 0);
	assert(fibocom_l850_set_response_is_success(success,
		strlen(success)));
	assert(!fibocom_l850_set_response_is_success("OK", 2U));
	assert(!fibocom_l850_set_response_is_success(success,
		strlen(success) - 1U));

	assert(fibocom_l850_state_transition_is_valid(
		FIBOCOM_L850_STATE_AVAILABLE, FIBOCOM_L850_STATE_SCAN_READY));
	assert(fibocom_l850_state_transition_is_valid(
