...
main(void)
{
	...
	static const char embedded[] = "l850gl-safe\0hidden";

	blob_buf_init(&buffer, 0);
	add_valid_fields(&buffer);
	assert(l850gl_ubus_parse_exact(buffer.head, policy, __FIELD_MAX,
		required, parsed));
	...
	blob_buf_init(&buffer, 0);
	assert(blobmsg_add_string(&buffer, "modem_id", "l850gl-valid") == 0);
	...
	assert(!l850gl_ubus_blob_attr_is_valid(attribute,
	...
	blob_buf_init(&buffer, 0);
	assert(l850gl_ubus_message_is_empty(buffer.head));
	assert(blobmsg_add_string(&buffer, "ubus_rpc_session",
		"0123456789abcdef0123456789abcdef") == 0);
	assert(l850gl_ubus_message_is_empty(buffer.head));
	assert(blobmsg_add_string(&buffer, "ubus_rpc_session",
