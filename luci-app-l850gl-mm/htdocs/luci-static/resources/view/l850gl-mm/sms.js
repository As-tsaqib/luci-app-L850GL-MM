...
function legacyCopyText(value) {
	return new Promise(function(resolve, reject) {
		...
		buffer.className = 'l850gl-mm-copy-buffer';
		buffer.value = value;
...
function copyableMessage(value) {
	...
	while ((match = pattern.exec(value)) !== null) {
		...
		output.push(E('button', {
			'class': 'l850gl-mm-sms-number l850gl-mm-sms-interactive',
			'type': 'button',
			'title': _('Copy number'),
...
function trashIcon() {
	return E('span', {
		'class': 'l850gl-mm-sms-trash-glyph',
		'aria-hidden': 'true'
	}, []);
}
...
function renderMessage(controller, entry, state, message) {
	...
	const cardClasses = [ 'cbi-section-node', 'l850gl-mm-sms-card' ];
	let pressTimer = null;
	let pressX = null;
	let pressY = null;
	const rows = [
	...
	if (number)
		cardClasses.push('l850gl-mm-sms-card-clickable');
	if (selected)
		cardClasses.push('l850gl-mm-sms-card-selected', 'is-selected');
	if (state.selectionMode)
		cardClasses.push('l850gl-mm-sms-select-mode');
	if (state.conversationNumber)
		cardClasses.push(isIncoming(direction) ?
			'l850gl-mm-sms-chat-inbound' : 'l850gl-mm-sms-chat-outbound');
	...
	return E('div', {
		...
		E('div', { 'class': 'l850gl-mm-sms-card-tools' }, [
			selected ? E('span', {
				'class': 'l850gl-mm-sms-selection-check',
				'title': _('Selected'),
				'aria-label': _('Selected')
			}, [ '\u2713' ]) : E([]),
			smsId ? E('button', {
				'class': 'btn cbi-button cbi-button-negative l850gl-mm-sms-delete-icon l850gl-mm-sms-interactive',
				'type': 'button',
				'title': _('Delete SMS'),
				...
				'class': 'l850gl-mm-sms-delete-progress',
				'aria-hidden': 'true'
			}, [ '\u2026' ]) : trashIcon() ]) : E([])
		]),
		widgets.keyValueList(rows),
		E('div', { 'class': 'l850gl-mm-sms-body' }, [
			E('div', { 'class': 'l850gl-mm-sms-body-title' }, [ _('Message'), ':' ]),
			E('div', { 'class': 'l850gl-mm-sms-message' }, copyableMessage(body))
		]),
...
function renderComposeButton(controller, state) {
	return E('button', {
		'class': 'btn cbi-button cbi-button-neutral l850gl-mm-compose-toggle',
		'type': 'button',
...
function renderCompose(controller, entry, state, index) {
	const busy = isMutationBusy(state);
	const recipientId = 'l850gl-mm-sms-recipient-' + index;
	const textId = 'l850gl-mm-sms-text-' + index;
	const children = [
		E('div', { 'class': 'cbi-value l850gl-mm-form-row' }, [
			E('label', {
		...
		E('div', { 'class': 'cbi-page-actions l850gl-mm-actions' }, [
			E('button', {
	...
	return E('form', {
		'class': 'cbi-section l850gl-mm-panel l850gl-mm-compose l850gl-mm-compose-form',
...
function renderConversationCompose(controller, entry, state, index) {
	const busy = isMutationBusy(state);
	const textId = 'l850gl-mm-sms-conversation-text-' + index;

	return E('form', {
		'class': 'l850gl-mm-compose l850gl-mm-conversation-compose',
		'submit': function(event) {
			return handleSend(controller, entry, state, event);
		}
	}, [
		E('textarea', {
			'id': textId,
			'class': 'cbi-input-textarea l850gl-mm-conversation-input',
			'rows': 2,
			'autocomplete': 'off',
			'placeholder': _('Enter message text'),
		...
		E('button', {
			'class': 'btn cbi-button cbi-button-positive important l850gl-mm-conversation-send',
			'type': 'submit',
			'disabled': busy ? '' : null
		}, [ state.sending ? _('Sending…') :
			(state.clientToken ? _('Retry SMS send') : _('Send SMS')) ])
...
function renderConversationHeader(controller, state, messages) {
	return E('div', { 'class': 'cbi-section l850gl-mm-panel l850gl-mm-conversation-header' }, [
		E('button', {
			'class': 'btn cbi-button cbi-button-neutral l850gl-mm-conversation-back',
			'type': 'button',
			'disabled': isMutationBusy(state) ? '' : null,
			'click': function() { return closeConversation(controller, state); }
		}, [ _('Back') ]),
		E('div', { 'class': 'l850gl-mm-conversation-heading' }, [
			E('div', { 'class': 'l850gl-mm-conversation-title' }, [ _('Conversation') ]),
			E('div', { 'class': 'l850gl-mm-conversation-number' }, [ state.conversationNumber ])
		]),
		E('span', { 'class': 'label l850gl-mm-conversation-count' }, [
...
function renderMessageActions(controller, entry, state, messages, hasMore) {
	...
	if (!state.conversationNumber || state.selectionMode) {
		children.push(E('button', {
			'class': 'btn cbi-button cbi-button-negative l850gl-mm-sms-bulk-delete',
			'type': 'button',
	...
	if (state.selectionMode) {
		children.push(E('span', {
			'class': 'l850gl-mm-sms-selected-count',
			'aria-live': 'polite'
		}, [ _('%d selected').format(selected.length) ]));
		children.push(E('button', {
			'class': 'btn cbi-button cbi-button-neutral l850gl-mm-sms-selection-cancel',
			'type': 'button',
			'disabled': busy ? '' : null,
			'click': function() {
				clearSelection(state);
	...
	if (hasMore && state.pageCount < MAX_SMS_PAGES) {
		children.push(E('button', {
			'class': 'btn cbi-button cbi-button-neutral l850gl-mm-sms-load-more',
			'type': 'button',
	...
	return children.length ?
		E('div', { 'class': 'cbi-page-actions l850gl-mm-sms-list-actions' }, children) :
		E([]);
}
...
function renderDevice(controller, entry, index, deviceCount) {
	...
	if (error) {
		if (deviceCount > 1) {
			children.push(E('div', { 'class': 'l850gl-mm-sms-device-label' }, [
				widgets.display(summary.model, _('L850-GL modem'))
			]));
		}
	...
	const filterId = 'l850gl-mm-sms-folder-' + index;
	...
	if (state.conversationNumber)
		...
		children.push(E('div', { 'class': 'l850gl-mm-sms-toolbar' }, [
			deviceCount > 1 ? E('span', {
				'class': 'l850gl-mm-sms-device-label'
			}, [ widgets.display(summary.model, _('L850-GL modem')) ]) : E([]),
			E('select', {
				'id': filterId,
				'class': 'cbi-input-select l850gl-mm-sms-filter-select',
				'aria-label': _('Folder'),
			...
			E('span', {
				'class': 'label notice l850gl-mm-sms-loaded-count',
				'aria-live': 'polite'
	...
	children.push(E('div', { 'class': 'cbi-section l850gl-mm-panel' }, [
		E('div', { 'class': 'l850gl-mm-sms-panel-heading' }, [
			E('h4', { 'class': 'l850gl-mm-panel-title' }, [
				state.conversationNumber ? _('Conversation') : _('Messages')
			]),
			!state.selectionMode ? E('span', {
				'class': 'cbi-value-description l850gl-mm-sms-select-hint'
			}, [ _('Hold a message to select it.') ]) : E([])
		]),
		visible.length ? E('div', {
			'class': 'l850gl-mm-sms-list' +
				(state.conversationNumber ? ' l850gl-mm-sms-chat-list' : '')
...
function renderSmsHeader(snapshot) {
	...
	return E('div', { 'class': 'l850gl-mm-sms-page-heading' }, [
		E('h2', {}, [ _('SMS') ]),
		E('span', {
			'class': 'l850gl-mm-sms-status-dot is-' + status.state,
			'role': 'status',
			'aria-live': 'polite',
...
function editorHasFocus(content) {
	...
	const compose = active && typeof active.closest === 'function' ?
		active.closest('.l850gl-mm-compose') : null;
...
return view.extend({
	load: loadSnapshots,
		...
		controller.header = E('div', { 'id': 'l850gl-mm-sms-header' }, [
		...
		return E('div', { 'class': 'cbi-map l850gl-mm-page l850gl-mm-sms-page' }, [
			widgets.stylesheet(),
			controller.header,
